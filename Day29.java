...
class Solution {
    ...
    public int maxStability(int n, int[][] edges, int k) {
        ...
        DSU dsuInit = new DSU(n);
        int mustMin = Integer.MAX_VALUE;
        int selected = 0;

        for (int[] e : mustEdges) {
            if (!dsuInit.union(e[0], e[1])) return -1;
            mustMin = Math.min(mustMin, e[2]);
            selected++;
        }

        if (selected > n - 1) return -1;
        ...
        while (left <= right) {
            ...
            DSU dsu = new DSU(n);
            System.arraycopy(dsuInit.parent, 0, dsu.parent, 0, n);
